...
2. Master (Shard分配逻辑)
   ...
   ​	借鉴6.824的设计。Master提供的是对metadata的维护。生成从shardID -> groupID的配置，动态的将所有shard均匀分配到不同的slave服务器组。
...
5. Slave_Server
   ...
   - **在Master改变shard配置的时候如何对应的移动本地的localstorage**
     ...
     - 引入新的RPC call
       ...
       允许通过调用节点A的TransferShard将自己的一个localstorage传递给节点A。如果节点A是primary的话，还会同步给他的backup
...
- **语言**： **go** [version go1.14.2 darwin/amd64]
  - RPC框架 gRPC
  - zookeeper client 使用github.com/samuel/go-zookeeper/zk
- **系统环境**
...
# maybe --build-arg http_proxy=http:ip:port
```
...
up后即可在localhost:2181, localhost:2182, localhost:2183分别访问到三个zookeeper节点。
